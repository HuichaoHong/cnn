...
void ConvLayer::forward(array3 input_array) {
	this->input_array = input_array;
	this->padded_input_array = padding(input_array,this->zero_padding);
	for (size_t f = 0; f < this->filter_number; f++)
	{
		conv(this->padded_input_array,Filters[f].get_weights(),this->output_array,1,Filters[f].get_bias());
	}
...
void ConvLayer::bp_sensitivity_map(array3 sneditivity_array, Activator activate){
	...
	int expand_width=expand_sensitivity_array[0][0].size();
	int zp = (this->input_width + this->filter_width - 1 - expand_width) / 2;
	array3 padded_array = padding(expand_sensitivity_array,zp);
	...
	for (size_t f = 0; f < this->filter_number; f++)
	{
		...
		array3 flipped_weights = matrix_rot180(filter.get_weights());
		array3 delta = create_delta_array();
...
array3 ConvLayer::matrix_rot180(array3 input_array){
	...
	for (size_t i = 0; i < input_array.size(); i++)
	{
		for (size_t j = 0; j < input_array[0].size(); j++)
		{
			...
			{
				output_array[i][input_array[0].size() - j][input_array[0][0].size() - k] = input_array[i][j][k];
